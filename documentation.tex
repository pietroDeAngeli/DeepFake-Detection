...
The dataset is structured to in subdirectories, the FF++ dataset I downloaded by applying the pipeline described in the next section. The result has been stored by saving a directory for each video where we can find (for each video):
\begin{itemize}
  ...
  \item \texttt{tensor.pt}: containig the stack of \texttt{motion_vector_x},\texttt{motion_vector_y},\texttt{information_mask} for each frame.
  \item \texttt{faces}: in this directory there are the images of the faces detected in the frames.
\end{itemize}

There is also the file \texttt{manifest.json} containing 2 objects: train and test where each entry contains the name of the directory of the file and the label. The size of the two sets are 70\% for the train set and 30\% for the test set. 
% To here

\subsection{\texttt{models}}

\begin{itemize}
  \item \textbf{YuNet}: a pretrained face detector returning face bounding boxes and confidence scores. This model is the bottleneck of our system.
  \item \textbf{Classifier}: This model is based on MobileNetV3 that takes both RGB and MV+IM features and returns the prediction so it's a two streams architecture that has been modified from the base version because of the input and output layer since the first has been adapted to handle 3 channels data for both streams (one for the RGB and one for mv_x,mv_y, IM) and the last layer to combine the results of the two networks. The paper just average on the two results but I decided to introduce a new parameter \texttt{alpha} to apply a weighted sum of the two networks. %TODO siamo sicuri di voler allenare un altro parametro? non penso che sia un gran dramma dal momento che ne alleno già parecchi
\end{itemize}

% fix from here
\subsection{\texttt{src}}

In this directory there is all the python code of the project, it's organized in directories that I'll described in the next subsections.

\subsubsection{\texttt{Classifier}}
% TODO
In this directory is contained the preprocess of the network to adapt it to a two streams and the training file I will describe later. 
% should I write more?

\subsubsection{\texttt{data_preprocessing}}
In this directory I describe the data preprocessing on the video and to split it. I call the function \texttt{create_dataset()} where for each video I run the pipeline and created the files and directories as described in the dataset section. Then I used the function \texttt{split_dataset()} to create the file \texttt{manifest.json}.
...
In this directory there are the tests of the pipeline described below. They are just tests fo the result without the testing the whole pipeline. 
% Should i write more?
% to here
...
% fix from here
\subsection{Tools}
In this file there is the function \texttt{get\_dir\_videos()} that could be helpful in every file and returns a list of the paths of all the the .mp4 files contained in the directory specified in the parameter \texttt{path}.
% To here
...
The training has been done on 280 videos and tested on 120 and they are defined in the file \texttt{manifest.json}. 

I used an Adam optimizer with learning rate set to ?? and trained on ?? epoches. poi non lo so. 
